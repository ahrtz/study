...
v <- welfare %>% group_by(gender)%>% summarise(income=mean(avg_inc,na.rm = T) )

welfare %>%  group_by(gender)%>% summarise(mean_income = mean(avg_inc,na.rm = T))
#그래프로 보여주기 캡션이랑 타이틀 행렬 이름 주는거 잘 보기 
ggplot(gender_income,aes(x=gender,y=mean_income))+geom_col(width = 0.3)+ labs(x="성별",y=" 평균월급",title = "성별에 따른 월급",subtitle = "부제목",caption = "example 1 Fig")
